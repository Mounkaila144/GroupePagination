    ...
    <div class="row">
        <section class="col-12">
            ...
            <form action="index.php" method="post">
                <p>groupe : <input type="number" name="groupe" /></p>
            ...
            <nav>
                <ul class="pagination">
                    <!-- Lien vers la page précédente (désactivé si on se trouve sur la 1ère page) -->
                    <li class="page-item">
                        <a href="?pagemin=<?= $Groupe->getPreviewroupe()[0] ?>&pagemax=<?= $Groupe->getPreviewroupe()[1] ?>" class="page-link">Précédente</a>
                    </li>
                    <?php foreach($Groupe->getAllGroupes() as $pages){ ?>
                        <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->

                        <a href="?pagemin=<?= $pages[0] ?>&pagemax=<?=$pages[1] ?>&groupe=<?= $groupe ?>" class="page-link  <?= ($pages[0]== 121 and $pages[1]==160) ? "active" : "active" ?>">
                            <?= $pages[0] ?> a <?= $pages[1] ?>
                        </a>
                    ...
                    <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
                    <li class="page-item">
                        <a href="?pagemin=<?= $Groupe->getNextGroupe()[0] ?>&pagemax=<?= $Groupe->getNextGroupe()[1] ?>" class="page-link">Suivante</a>
                    </li>
                </ul>
            </nav>
        </section>
